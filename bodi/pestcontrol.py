# this code is mostly taken from COMBO (Oh et al.) paper
...
class PestControl(TestFunction):
    ...
    def __init__(self, random_seed=0,
                 normalize=False, **tkwargs):
        super(PestControl, self).__init__(normalize)
        self.n_binary = 0
        self.binary_inds = []
        self.n_continuous = 0
        self.continuous_inds = []
        self.n_categorical = PESTCONTROL_N_STAGES
        self.categorical_inds = list(range(PESTCONTROL_N_STAGES))
        self.dim = self.n_categorical
        self.bounds = torch.stack((torch.zeros(PESTCONTROL_N_STAGES),  (PESTCONTROL_N_CHOICE-1) * torch.ones(PESTCONTROL_N_STAGES))).to(**tkwargs)
    ...
    def _compute(self, x, normalize=False):
